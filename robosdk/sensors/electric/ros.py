...
class RosBatteryDriver(RosSensorBase, ElectricBase):  # noqa
    ...
    def __init__(self, name, config: Config = None):
        ...
        if (hasattr(self.config, "info") and
                hasattr(self.config.info, "target")):
    ...
    def _battery_info_callback(self, info: Any):
        if info is not None:
            try:
                ...
                self.logger.error(f"get data from [{self.config.info.target}]"
                                  f" fail: {str(e)}")
        else:
            self.electric_info = {}
    ...
    def close(self):
        if self.info_sub:
            self.backend.unsubscribe(self.info_sub)
